...
@export var grid_size := Vector2i(26, 26)
...
func _make_looping_player(stream: AudioStream, volume_db := -8.0, pitch_scale := 1.0, max_distance := 40.0, autoplay := true) -> AudioStreamPlayer3D:
    if stream is AudioStreamWAV and stream.loop_mode == AudioStreamWAV.LOOP_DISABLED:
        var loopable: AudioStreamWAV = stream.duplicate()
        loopable.loop_mode = AudioStreamWAV.LOOP_FORWARD
        stream = loopable
...
func _build_factory_blockout():
    ...
    _add_factory_ambience(playfield)
    _prune_spawn_positions(playfield, nav_blockers)
...
func _create_conveyor_belt(def: Dictionary, accent_material: StandardMaterial3D, tread_material: StandardMaterial3D):
    ...
    var conveyor_audio := _make_looping_player(AudioFactory.factory_hum(), -9.0, 0.9, 48.0)
...
func _create_press(def: Dictionary, accent_material: StandardMaterial3D):
    ...
    var hydraulic_hum := _make_looping_player(AudioFactory.factory_hum(), -7.4, 0.62, 36.0)
    hydraulic_hum.position = Vector3(0, 1.4, 0)
    root.add_child(hydraulic_hum)

    var slam_clang := _make_looping_player(AudioFactory.factory_alarm(), -5.2, 0.9, 28.0, false)
...
func _create_robotic_arm(def: Dictionary, accent_material: StandardMaterial3D):
    ...
    var arc_sound := _make_looping_player(AudioFactory.factory_arc(), -6.0, 1.1, 28.0, false)
...
func _create_furnace(def: Dictionary, accent_material: StandardMaterial3D, secondary_material: StandardMaterial3D):
    ...
    var siren := _make_looping_player(AudioFactory.factory_alarm(), -10.5, 0.58, 60.0)
    siren.position = Vector3(0, height * 0.6, 0)
    root.add_child(siren)

    var heat_rumble := _make_looping_player(AudioFactory.factory_hum(), -8.0, 0.98, 46.0)
...
func _add_factory_ambience(playfield: Vector2):
    var hum_sites := [
        Vector2(playfield.x * 0.5, playfield.y * 0.5),
        Vector2(playfield.x * 0.3, playfield.y * 0.18),
        Vector2(playfield.x * 0.78, playfield.y * 0.82),
    ]
    for site in hum_sites:
        var bed := _make_looping_player(AudioFactory.factory_hum(), -11.5 + rng.randf_range(-1.2, 0.6), 0.9 + rng.randf_range(-0.06, 0.08), 160.0)
        bed.position = Vector3(site.x, 1.5, site.y)
        level_root.add_child(bed)

    var pulse := _make_looping_player(AudioFactory.factory_alarm(), -14.0, 0.85, 180.0)
    pulse.position = Vector3(playfield.x * 0.62, 2.2, playfield.y * 0.44)
    level_root.add_child(pulse)

    var arc_beacon := _make_looping_player(AudioFactory.factory_arc(), -9.0, 1.08, 120.0)
    arc_beacon.position = Vector3(playfield.x * 0.42, 1.6, playfield.y * 0.8)
    level_root.add_child(arc_beacon)

func _prune_spawn_positions(playfield: Vector2, blockers: Array):
    var safe_bounds := Rect2(1.5, 1.5, playfield.x - 3.0, playfield.y - 3.0)
    var filtered_spawns: Array = []
    for spot in spawn_positions:
        var planar := Vector2(spot.x, spot.z)
        if not safe_bounds.has_point(planar):
            continue
        if _is_point_blocked(planar, blockers, 1.2):
            continue
        filtered_spawns.append(spot)

    if filtered_spawns.is_empty():
        filtered_spawns.append(_choose_center_spawn())
    spawn_positions = filtered_spawns

    var filtered_floors: Array = []
    for floor in floor_positions:
        var planar := Vector2(floor.x, floor.z)
        if not safe_bounds.has_point(planar):
            continue
        if _is_point_blocked(planar, blockers, 0.8):
            continue
        filtered_floors.append(floor)

    if not filtered_floors.is_empty():
        floor_positions = filtered_floors

func _is_point_blocked(point: Vector2, blockers: Array, margin := 0.0) -> bool:
    for blocker in blockers:
        var expanded: Rect2 = blocker.grow(margin)
...
func spawn_pickups():
    ...
    var pickups_to_spawn: int = clamp(int(grid_size.x * grid_size.y / 70.0), 4, 10)
    var placed_positions: Array = []

    for i in range(pickups_to_spawn):
        var pickup = pickup_scene.instantiate()
        var weapon_data: Dictionary = WeaponData.WEAPONS[rng.randi_range(0, WeaponData.WEAPONS.size() - 1)]
        pickup.weapon_id = weapon_data.get("id", "pistol")
        pickup.display_name = weapon_data.get("name", pickup.weapon_id.capitalize())

        var spot: Vector3 = get_random_floor_position(6.0)
        var attempts := 0
        while attempts < 10:
            var valid := true
            for placed in placed_positions:
                if placed.distance_to(spot) < 4.0:
                    valid = false
                    break
            if valid:
                break
            spot = get_random_floor_position(6.0)
            attempts += 1

        placed_positions.append(spot)
        pickup_root.add_child(pickup)
        pickup.global_transform.origin = spot

func spawn_enemies():
    if enemy_scene == null:
        return
    var area: float = float(grid_size.x * grid_size.y)
    var melee_count: int = clamp(int(area / 40.0), 6, 18)
    var ranged_count: int = clamp(int(area / 70.0), 3, 12)

    for i in range(melee_count):
        _spawn_enemy(enemy_scene)
    var ranged_scene := ranged_enemy_scene if ranged_enemy_scene else enemy_scene
    for i in range(ranged_count):
        _spawn_enemy(ranged_scene)
...
func _spawn_enemy(scene: PackedScene):
    ...
    enemy.target = player
    var enemy_position: Vector3 = get_random_floor_position(tile_size * 2.5) + Vector3(0, 0.5, 0)
